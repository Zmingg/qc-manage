<template>
    <div class="main">
        ...
        <div class="disc-form" v-show="creating || updating">
            ...
            <el-input class="input" v-model="disc.count">
            ...
            <el-button class="input" type="success" v-show="creating" @click="saveDisc">添加</el-button>
            <el-button class="input" type="primary" v-show="updating" @click="updateDisc">修改</el-button>
            <el-button class="input" type="danger" v-show="updating" @click="deleteDisc">删除</el-button>
            <el-button class="input" @click="reset">取消</el-button>
        </div>
        <el-button v-show="!creating&&!updating" class="input" type="success" @click="createDisc">添加新专辑</el-button>
    </div>
</template>
<script>
import api from '../../api/music/disc';
import { apiSingers } from '../../api/music';
export default {
    data(){
        return {
            singers: [],
    ...
    methods: {
        ...
        updateDisc: async function () {
            let res = await api.update(this.disc);
            if (res.ok) {
                this.getAllDiscs();
                this.reset();
            }
        ...
        deleteDisc: async function () {
            let res = await api.delete(this.disc.did);
            if (res.ok) {
                this.getAllDiscs();
                this.reset();
            }
        ...
        saveDisc: async function () {
            ...
            let res = await api.create(this.disc);
            if (res.ok) {
                this.getAllDiscs();
                this.reset();
            }
